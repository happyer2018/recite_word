...
public final class RomUtil {
    ...
    public static boolean isMiui9Below() {
        try {
            String miCode = getProp(KEY_MIUI_VERSION_CODE);
            if (!TextUtils.isEmpty(miCode)) {//MIUI9 = 7
                return Integer.valueOf(miCode) < 7 || Build.VERSION.SDK_INT < Build.VERSION_CODES.M;
            }
        } catch (Exception e) {
    ...
    public static boolean isFlyme() {
        ...
        try {
            String model = getProp("ro.meizu.product.model");
            firstCheck = (!TextUtils.isEmpty(model))
                    || "meizu".equalsIgnoreCase(Build.BRAND)
                    || "22c4185e".equalsIgnoreCase(Build.BRAND);
        } catch (Exception e) {
        }
        return firstCheck || check(ROM_FLYME);
    }
